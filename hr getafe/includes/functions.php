...
function query_fetch($conn, $sql, $params = [], $types = '') {
    if (empty($params)) {
...
// Get employee by ID
function get_user_employee($conn, $user_id) {
    $result = $conn->query("SELECT employee_id FROM users WHERE user_id = $user_id");
    return $result->fetch_assoc()['employee_id'];
}
?>
